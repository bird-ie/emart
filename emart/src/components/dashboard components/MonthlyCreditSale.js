...
export default function MonthlyCreditSale() {
  ...
  useEffect(() => {
    const fetchData = () => {
      fetch("https://crystalsolutions.com.pk/emart/web/MonthlyCreditSale.php", {
        method: "POST",
        headers: {
        ...
        .then((data) => {
          console.log('getting monthly credit sale data',data.detail);
  ...
  return (
    <Container>
      <Row className="bg-white shadow p-2 flex-row align-items-center">
        <h6 style={{ color: 'var(--primary-color)' }}>Monthly Credit Sale</h6>
        <Col>
          <ReactApexChart
